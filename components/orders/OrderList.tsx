      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {['ALL', ...Object.values(OrderStatus)].map(s => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-4 py-1 rounded-full text-xs font-bold whitespace-nowrap transition-all ${filter === s ? 'bg-primary text-white' : 'bg-white border text-gray-400 hover:border-primary'
              }`}
          >
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-black text-gray-800">#{order.id.substring(0, 8).toUpperCase()}</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div onClick={(e) => e.stopPropagation()}>
                  <DescargarPDFButton pedido={order} variant="icon" />
                </div>
              </div>
            </div>
          ))
